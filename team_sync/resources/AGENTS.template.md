...
`python3 bin/team_sync.py start`.
...
| shared drive | large files | resolve the path with `python3 bin/team_sync.py paths`; never hard-code it |
| database | leases (mutual exclusion) and shared cursors | `python3 bin/team_sync.py lease ...` / `cursor ...` |
...
2. When you finish a piece of work: `python3 bin/team_sync.py note "what / where / next"`.
3. Keep large files on the drive, not in git. If the fingerprinted data directory changed: `python3 bin/team_sync.py fingerprint`.
